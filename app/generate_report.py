...
# Output to ./output per user preference (avoid C: drive for deliverables)
OUTPUT_DIR = os.environ.get("WORKBUDDY_OUTPUT", "./output")
...
function renderAdvCot(sec){
  ...
  var html='<div class="tt">CFTC COT 持仓 (非商业) <span style="float:right">'+_advBadge(sec.live,sec.fetched_at,sec.updated)+'</span></div>';
  var d=sec.data||{};
  if(d.items&&d.items.length){
...
function renderAdvEiaOil(sec){
  ...
  var html='<div class="tt">EIA 原油库存周度 <span style="float:right">'+_advBadge(sec.live,sec.fetched_at)+'</span></div>';
  el.innerHTML=html;
  var d=sec.data||{};
  var key=Object.keys(d).filter(function(k){return k!=='labels';})[0];
  if(d.labels&&key&&d[key]){
    var sm={};sm[key]=d[key].map(function(v,i){return [d.labels[i],v];});
    initAdvChart('advEiaOil','',sm);
  }
}
