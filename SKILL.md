...
daily_data.json  ──┐  (夜间批处理生成/更新, 唯一数据入口)
                    ├──▶  generate_report.py  ──▶  ./output\全球金融日报_<日期>.html (+ _V9.xlsx)
...
- HTML 服务：`Handler._html()` 每次 GET 都从 `./output` 读取**最新** `全球金融日报_*.html`（排除含 `v7_1`/`backup` 的旧文件），不缓存；所有 API 响应加 `no-cache` 头。
...
- **输出位置**：生成文件统一到 `./output`（用户偏好，禁写 C 盘）；预览 `http://localhost:8800/`。可用环境变量 `WORKBUDDY_OUTPUT`（HTML/Excel）、`WORKBUDDY_DATA_DIR`（daily_data.json）覆盖。
...
# 输出: ./output\全球金融日报_<日期>.html (+ _V9.xlsx)
